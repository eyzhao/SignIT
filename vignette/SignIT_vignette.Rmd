...
print(example_catalog)
```

SignIT also provides tools to determine SNV mutation catalogs from a table of variants. SignIT also provides example data of a variant table.

```{r variant_table_to_catalog}
...
example_catalog_2 <- with(
  example_variants,
  signit:::mutations_to_catalog(chr, pos, ref, alt)
)

print(example_catalog_2)
```
...
```{r signature_summary}
print(get_exposure_summary_table(patient_signature_exposures))
```
...
print(get_exposure_summary_table(sv_exposures))
```
...
population_signatures <- readRDS('../../../meta/mutation_timing/signit_pop_cases/POG042/biop1_t_P00019_blood1_n_P00018/signit_pop_output/2_populations.Rds')$stan_obj
```
...
print(summarise_population_signatures(population_signatures))
```
